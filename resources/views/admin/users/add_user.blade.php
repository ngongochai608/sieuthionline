...
<div class="col-lg-12">
    <div class="card">
        <div class="card-header">
            <strong>Thêm tài khoản quản trị viên</strong>
        </div>
        <?php
        ...
        <div class="card-body card-block">
            <form action="{{URL::to('/save-user')}}" method="post" enctype="multipart/form-data" class="form-horizontal">
                ...
                <div class="row form-group">
                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Tên quản trị viên</label></div>
                ...
                <div class="row form-group">
                    ...
                    <div class="col-12 col-md-6"><input type="password" name="admin_password_confirm" class="form-control">
                    @if($errors->has('admin_password_confirm'))
                        <p class="alert alert-danger">{{ $errors->first('admin_password_confirm') }}</p>
                    @endif</div>
                </div>
                <div class="row form-group">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-12 col-md-6"><input type="text" name="admin_address" class="form-control">
                    @if($errors->has('admin_address'))
                        <p class="alert alert-danger">{{ $errors->first('admin_address') }}</p>                  
                    @endif</div>
                </div>           
                <div class="row form-group">
                    <input style="margin: 0px auto;" type="submit" name="" value="Thêm quản trị viên" class="btn btn-primary btn-sm" onclick="return confirm('Bạn có chắc là muốn thêm tài khoản quản trị viên này không?')">
                </div>
            </form>
        </div>
    </div>
